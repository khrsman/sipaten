...
class Pamong extends CI_Controller {
    ...
    function data(){
    $this->load->view('v_pamong_data');
    }

    function editor(){
    $this->load->view('v_pamong_editor');
    }
    ...
    public function page()
    {
      ...
      $data = $this->M_pamong->get_data($search,$from_page,$per_page); // ambil data
    ...
    function add(){
        ...
        if (isset($data['id_pamong'])) {
          $data['id_pamong'] = NULL;
        };
        $insert = $this->M_pamong->insert($data);
        if (!$insert) {
    ...
    function update(){
      ...
      parse_str($_POST['data'], $data);
      $id = $data['id_pamong'];
      $update = $this->M_pamong->update_by_id($data,$id);
    }

    // fungsi hapus
    function delete(){
      $id = $this->input->post('id');
      $delete = $this->M_pamong->delete_by_id($id);
    }
