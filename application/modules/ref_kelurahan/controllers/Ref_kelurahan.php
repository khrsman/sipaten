...
class Ref_kelurahan extends CI_Controller {

    function __construct()
    {
        parent::__construct();
        $this->load->model('M_ref_kelurahan');
    }

    // view
    function index(){
    $data['page'] = 'v_ref_kelurahan';
    $data['data'] = $this->M_ref_kelurahan->get_data('',0,100); // ambil data
    ...
    public function page()
    {
      ...
      $data = $this->M_ref_kelurahan->get_data($search,$from_page,$per_page); // ambil data
    ...
    function data(){
    $this->load->view('v_ref_kelurahan_data');
    }

    function editor(){

    $this->load->view('v_ref_kelurahan_editor');
    }

    function get_for_edit(){
      $id = $this->input->get('id');
      $data = $this->M_ref_kelurahan->get($id);
    ...
    function add(){
        ...
        // if (isset($data['id_kelurahan'])) {
        //   $data['id_kelurahan'] = NULL;
        // };
        $insert = $this->M_ref_kelurahan->insert($data);
        if (!$insert) {
    ...
    function update(){
      ...
      $update = $this->M_ref_kelurahan->update_by_id($data,$id);
    }

    // fungsi hapus
    function delete(){
      $id = $this->input->post('id');
      $delete = $this->M_ref_kelurahan->delete_by_id($id);
    }
