...
class Jenis_surat extends CI_Controller {
    ...
    function data(){
    $this->load->view('v_jenis_surat_data');
    }

    function editor(){
    $data['field_isi'] = $this->M_jenis_surat->get_field_surat();
    $data['persyaratan'] = $this->M_jenis_surat->get_persyaratan();
    $this->load->view('v_jenis_surat_editor',$data);
    }
    ...
    function update(){
      ...
      parse_str($_POST['data'], $data);
      // print_r($data);
      $field_isi= $persyaratan = '';
      foreach ($data['field_isi'] as $key => $value) {
        $field_isi .= $value.",";
      }
      foreach ($data['persyaratan'] as $key => $value) {
        $persyaratan .= $value.",";
      }
      $data['field_isi'] = rtrim($field_isi,",");
      $data['persyaratan'] = rtrim($persyaratan,",");
